    ...
    for sim in SIMS:
        for scenario in SCENARIOS:
            ...
            submit_file = os.path.join(
                submit_dir, f"pz_challenge_taskset_4_{sim}_pz_estimate_{scenario}.hdf5"
            )
            model_file = os.path.join(
                submit_dir, f"pz_challenge_taskset_4_{sim}_pz_model_{scenario}.pkl"
            )
            training_file = os.path.join(
                public_area, f"pz_challenge_taskset_4_{sim}_training_{scenario}.hdf5"
            )
            test_file = os.path.join(
                public_area, f"pz_challenge_taskset_4_{sim}_test_{scenario}.hdf5"
            )
            output_file_2 = os.path.join(
                submit_dir,
                "outputs_2",
                f"pz_challenge_taskset_4_{sim}_pz_estimate_{scenario}.hdf5",
            )
            ...
            if run_taskset_4_estimation_only is not None:
                ...
                run_taskset_4_estimation_only(model_file, test_file, output_file_2)
            ...
            if run_taskset_4_training_and_estimation is not None:
